...
suite('Functional Tests', () => {
  ...
  test('Translation with text and locale fields', function(done) {
    chai.request(server)
    .post('/api/translate')
    .send({text: "anglicizing", locale: "american-to-british"})
    .end(function(err, res){
      assert.equal(res.status, 200)
      assert.isObject(res.body, 'response is an obj')
      assert.equal(res.body.text, "anglicizing")
      assert.equal(res.body.translation, '<span class="highlight">anglicising</span>')
      done()
      })
    })
  ...
  test('Translation with text and invalid locale field', function(done) {
    chai.request(server)
    .post('/api/translate')
    .send({text: "test text", locale: "invalid-locale"})
    .end(function(err, res){
      assert.equal(res.status, 200)
      assert.isObject(res.body, 'response is an obj')
